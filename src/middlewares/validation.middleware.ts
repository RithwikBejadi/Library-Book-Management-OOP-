...
class ValidationMiddleware {
  public validateBookCreate = (
    ...
    if (!author || typeof author !== "string" || author.trim() === "") {
      return res
        .status(400)
        .json({ message: "author is required and must be a non-empty string" });
    }

    if (!isbn || typeof isbn !== "string" || isbn.trim() === "") {
      return res
        .status(400)
        .json({ message: "isbn is required and must be a non-empty string" });
    }
    ...
    if (publishedYear < 1000 || publishedYear > new Date().getFullYear()) {
      return res
        .status(400)
        .json({ message: "publishedYear must be a valid year" });
    }

    if (!genre || typeof genre !== "string" || genre.trim() === "") {
      return res
        .status(400)
        .json({ message: "genre is required and must be a non-empty string" });
    }

    next();
  };

  public validateBookUpdate = (
    req: Request,
    res: Response,
    next: NextFunction,
  ) => {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "isbn must be a non-empty string" });
    }
    ...
    if (publishedYear !== undefined) {
      if (typeof publishedYear !== "number") {
        return res
          .status(400)
          .json({ message: "publishedYear must be a number" });
      }
      if (publishedYear < 1000 || publishedYear > new Date().getFullYear()) {
        return res
          .status(400)
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "genre must be a non-empty string" });
    }
